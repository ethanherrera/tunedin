...
const fixtureDirectory = new URL("../supabase/functions/music-catalog/fixtures/", import.meta.url);
...
Deno.serve({ hostname: "0.0.0.0", port }, async (request) => {
  ...
  if (!accept.includes("application/json") || !userAgent.startsWith("tunedIn/")) {
    return json({ error: "missing required fixture headers" }, 400);
  }

  const url = new URL(request.url);
  const match = url.pathname.match(
    /^\/ws\/2\/(artist|area|place|recording|series)(?:\/([0-9a-f-]+))?$/,
  );
  ...
  if (musicBrainzId !== undefined) {
    if (endpoint === "artist" && musicBrainzId === "11111111-1111-4111-8111-111111111111") {
    ...
    if (fixtureName === undefined) return json({ error: "fixture lookup not found" }, 404);
  ...
  if (descriptor === undefined) return json({ error: "fixture search not found" }, 404);
  const offset = parseOffset(url.searchParams.get("offset"));
  if (query.includes("fixture-empty")) {
    return json({ created: "2026-07-15T00:00:00Z", count: 0, offset, [descriptor.collection]: [] });
  }
  const payload = JSON.parse(await readFixture(descriptor.file)) as Record<string, unknown>;
...
function parsePort(value: string): number {
  ...
  if (!Number.isInteger(parsed) || parsed < 1_024 || parsed > 65_535) {
    throw new Error("MUSICBRAINZ_STUB_PORT must be an integer from 1024 through 65535.");
  }
  return parsed;
}
