...
- Local-stack helpers must route through `scripts/worktree-local-supabase.sh`, which gives each Git worktree a unique Docker project, port block, volumes, Storage/Auth/Inbucket state, and generated ignored project directory. They may read its `status-env` output to populate ignored local configuration, but must never print or log returned keys.
