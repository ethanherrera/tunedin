...
- Never commit screenshots or other visual-validation artifacts to the repository or include them in a pull request's file diff. When Ethan asks for screenshots, treat that as a request for externally attached images in the pull-request summary—not tracked files, raw links to this project's repository files, or a documentation change—unless he explicitly requests the opposite.
...
- `tunedin-dev` is the shared integration environment between disposable Local and protected Staging. It may advance from an explicitly requested feature-branch commit, including work that is not yet merged to `main`; it may be unstable while that work is under development.
- Development advances only through an explicitly dispatched Development workflow from the requested branch commit. The workflow must rerun disposable backend verification before mutation; pull-request checks and merge-triggered workflows remain non-mutating.
