...
- `tunedIn-Development` targets the shared hosted Development project. Its unsigned Simulator build persists a normal Development Auth session in app-scoped UserDefaults; physical devices use Keychain. `tunedIn-Local` uses `Local.xcconfig` generated from the disposable Docker stack; `make simulator-local` refreshes it automatically, or use `make configure-local-supabase` directly. Never paste local keys into source or a committed configuration.
- Run `make lint` and `make test`. Local build and test commands use the current Git worktree's isolated iPhone 13 Simulator and Derived Data; keep `DESTINATION` overrides available for CI. Use Xcode/Simulator for visual inspection, runtime logs, crashes, profiling, signing, or capabilities.
- After Simulator testing, leave the Simulator open with the app running on the last relevant verified screen so Ethan can continue testing manually. Do not terminate the app, shut down or erase the Simulator, or reset its journey state unless Ethan explicitly requests cleanup.
- When testing the temporary Development magic-link flow, copy the email button's link address and run `make simulator-auth-link` with tunedIn installed in this worktree's Simulator. Never paste, print, or commit the one-time Auth URL or token.
